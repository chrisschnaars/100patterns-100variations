...
int yCount = 10;;
float cellW, cellH;
int cellSpacer = 4; // experiment with 0 and 20
...
void draw() {
  if (iteration <= totalCount) {
    ...
    if (capture == true) {
      if (monochromatic) {
        beginRecord(PDF, "../../../OUTPUT/" + patternNumber + "/-mono/100p_" + patternNumber + "_mono.pdf");
      } else {
...
void keyPressed() {
  if (key == TAB) {
    save("../-outtakes/" + patternNumber + "_" + millis() + ".png");
  }
  // iteration = 1;
}
